...
def check_user_permission_nested(
    user: Dict[str, Any], resource: Dict[str, Any], action: str
) -> bool:
    """
...
def process_order_nested(order: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Contains deeply nested conditionals that make the logic hard to follow.
    """
    result = {"order_id": order.get("id"), "status": "pending", "messages": []}
                ...
                if "method" in payment:
                    if payment["method"] == "credit_card":
                        if "card_info" in payment:
                            ...
                            if (
                                "number" in card_info
                                and "expiry" in card_info
                                and "cvv" in card_info
                            ):
                                ...
                                result["messages"].append(
                                    "Incomplete credit card information"
                                )
...
def check_user_permission_better(
    user: Dict[str, Any], resource: Dict[str, Any], action: str
) -> bool:
    """
    Check if a user has permission to perform an action on a resource.
...
def _check_manager_permissions(
    user: Dict[str, Any], resource: Dict[str, Any], action: str, resource_type: str
) -> bool:
    ...
    if resource_type == ResourceType.DOCUMENT.value:
        ...
        if action in [Action.READ.value, Action.WRITE.value, Action.UPDATE.value]:
            ...
            if (
                ...
                and resource.get("visibility") == Visibility.PUBLIC.value
            ):
                return True
...
def _check_employee_permissions(
    ...
    if resource_type == ResourceType.DOCUMENT.value:
        ...
        if action == Action.READ.value:
            ...
            if visibility == Visibility.INTERNAL.value and _is_same_department(
                user, resource
            ):
                return True
...
def _apply_customer_discounts(
    customer: Dict[str, Any], total: float
) -> Dict[str, float]:
    ...
    # Loyal regular customer discount
        ...
        and customer.get("loyalty_years", 0) > 2
        and total > 200
    ):
        discount = total * 0.05
        result["discount"] = discount
